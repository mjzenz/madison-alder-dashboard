...
The % of votes that are pro-housing (YIMBY) for each active alder. The YIMBY score is calculated as the number of pro-housing votes divided by the total number of contested votes.  See @yimby-score for methodological details. This provides both a score for only this term, as well as an "all-time" score for all votes since 2023. For vote details of previous terms see [Historical Voting Record](councilytics.com\Historical_2023_25_Voting.html). Note that alders in even-numbered districts are up for reelection in Spring 2026. 

```{r}
#| label: tbl-YIMBY_percentage
#| tbl-cap: Percentage of contested votes by active alders that were pro-housing (YIMBY). "All-time" includes all votes since 2023.
#| echo: false   
#| warning: false
#| message: false

names(YIMBY.prop) <- c("Alder", "Aldermanic District", 
                       "n Votes", "% YIMBY", 
                       "n Votes ", "% YIMBY ")
kableExtra::kable(YIMBY.prop, "html") |>
  ...
  kableExtra::add_header_above(c(" " = 2, "2025 Term" = 2, "All Time" = 2)) |>
  kableExtra::column_spec(1, bold = TRUE, border_left = TRUE) |>
  kableExtra::column_spec(c(2,4,6), border_right = TRUE) |>
  kableExtra::column_spec(5:6, bold = TRUE) |>
  kableExtra::row_spec(0, bold = TRUE, color = "black", background = "#66cbff", extra_css = "padding: 7px 6px;") |>
  kableExtra::row_spec(1:nrow(YIMBY.prop), extra_css = "padding: 5px 6px; line-height: 1.1;")
```
...
kable(YIMBY.vote.list.2025, "html", escape = FALSE) %>%
  ...
  row_spec(0, bold = TRUE, color = "black", background = "#66cbff", extra_css = "padding: 7px 6px;") %>%
  row_spec(1:nrow(YIMBY.vote.list.2025), extra_css = "padding: 4px 6px; line-height: 1.1;") %>%
  column_spec(1:5, bold = FALSE) %>%
...
```{r}
#| label: tbl-all-other-votes-2025
#| tbl-cap: Contested Non-housing votes of active alders in 2025, listed by their district. Yes votes in blue, No votes in red.
#| echo: false
#| warning: false
#| message: false

#Only keep votes in YIMBY.vote.list from 2025
other.vote.list.2025 <- other.vote.list %>%
  ungroup() %>%
  filter(year(date) == 2025) %>%
  select(-date, -YIMBY) 


kable(other.vote.list.2025, "html", escape = FALSE) %>%
  kable_styling(
    bootstrap_options = "striped",
    full_width = FALSE,
    position = "center",
  ...
  row_spec(0, bold = TRUE, color = "black", background = "#66cbff", extra_css = "padding: 7px 6px;") %>%
  row_spec(1:nrow(other.vote.list.2025), extra_css = "padding: 4px 6px; line-height: 1.1;") %>%
  column_spec(1:4, bold = FALSE) %>%
  column_spec(5, color = ifelse(other.vote.list.2025$`1` == "Y", "blue", 
                                ifelse(other.vote.list.2025$`1` == "N", "red", "black"))) %>%
  column_spec(6, color = ifelse(other.vote.list.2025$`2` == "Y", "blue", 
                                ifelse(other.vote.list.2025$`2` == "N", "red", "black"))) %>%
  column_spec(7, color = ifelse(other.vote.list.2025$`3` == "Y", "blue", 
                                ifelse(other.vote.list.2025$`3` == "N", "red", "black"))) %>%
  column_spec(8, color = ifelse(other.vote.list.2025$`4` == "Y", "blue", 
                                ifelse(other.vote.list.2025$`4` == "N", "red", "black"))) %>%
  column_spec(9, color = ifelse(other.vote.list.2025$`5` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`5` == "N", "red", "black"))) %>%
  column_spec(10, color = ifelse(other.vote.list.2025$`6` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`6` == "N", "red", "black"))) %>%
  column_spec(11, color = ifelse(other.vote.list.2025$`7` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`7` == "N", "red", "black"))) %>%
  column_spec(12, color = ifelse(other.vote.list.2025$`8` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`8` == "N", "red", "black"))) %>%
  column_spec(13, color = ifelse(other.vote.list.2025$`9` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`9` == "N", "red", "black"))) %>%
  column_spec(14, color = ifelse(other.vote.list.2025$`10` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`10` == "N", "red", "black"))) %>%
  column_spec(15, color = ifelse(other.vote.list.2025$`11` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`11` == "N", "red", "black"))) %>%
  column_spec(16, color = ifelse(other.vote.list.2025$`12` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`12` == "N", "red", "black"))) %>%
  column_spec(17, color = ifelse(other.vote.list.2025$`13` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`13` == "N", "red", "black"))) %>%
  column_spec(18, color = ifelse(other.vote.list.2025$`14` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`14` == "N", "red", "black"))) %>%
  column_spec(19, color = ifelse(other.vote.list.2025$`15` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`15` == "N", "red", "black"))) %>%
  column_spec(20, color = ifelse(other.vote.list.2025$`16` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`16` == "N", "red", "black"))) %>%
  column_spec(21, color = ifelse(other.vote.list.2025$`17` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`17` == "N", "red", "black"))) %>%
  column_spec(22, color = ifelse(other.vote.list.2025$`18` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`18` == "N", "red", "black"))) %>%
  column_spec(23, color = ifelse(other.vote.list.2025$`19` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`19` == "N", "red", "black"))) %>%
  column_spec(24, color = ifelse(other.vote.list.2025$`20` == "Y", "blue", 
                                 ifelse(other.vote.list.2025$`20` == "N", "red", "black")))
